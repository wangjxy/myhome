...
public class RelativeKmeans {
	...
	//T S H E
//	private static int i1 = 19;
//	private static int i2 = 18;
//	private static int i3 = 7;
//	private static int i4 = 4;
//	private static int i5 = 176;
//	private static int i6 = 185;
	private static int dim = 10;
	private static int oNum = 214;
...
//    	oList[19] = new double[][]{{726,781},{614,758},{742,773},{615,746},{730,783},{630,779},{618,707},{705,780},{649,797},{640,734},{716,716},{701,768},{734,713},{642,736},{710,798}};
	    ...
	    for(int i=0;i<oList.length;i++){
	    	...
	    	oRange[i][0] = new Double[]{(xMin+xMax)/2,(xMin-xMax)/2};
	    	oRange[i][1] = new Double[]{(yMin+yMax)/2,(yMin-yMax)/2};
	    }
	    /*end样本点转区间数*/
...
//	    }
    	...
    	/*start读取数据内容并转换为联系数*/
/**    	File file = new File("D:"+File.separator+"dataset"+File.separator+"glass.txt");
//    	File rFile = new File("D:"+File.separator+"dataset"+File.separator+"wine-uncertain.txt");
    	BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(file)));
//    	BufferedReader br1 = new BufferedReader(new InputStreamReader(new FileInputStream(rFile)));
    	String msg = "";
    	int count = 0;
    	while((msg=br.readLine())!=null){
//    		String rStr = br1.readLine();
//    		String[] rData = rStr.split(",");
    		String[] strData = msg.split(",");
    		for(int q=0;q<strData.length-1;q++){
//    			oRange[count][q-1][0] = Double.valueOf(strData[q]);
//    			oRange[count][q-1][1] = Double.valueOf(rData[q-1]);
...
//	    System.out.println("F-Measure:"+obj.showEveResult(clusters,new int[]{69,145,162,175,184,214}));
//	    for(int i=0;i<points.length;i++){
...
//	    System.out.println(oRange[12][0][0]+"-"+oRange[12][0][1]+"-"+oRange[12][1][0]+"-"+oRange[12][1][1]+"-");
    ...
    /*根据中心点随机决定半径范围*/
    public double getR(double num){
    	Random rd = new Random();
		int max = (int)(num*0.8*100);
...
//    	}
    	...
    	for(int k=0;k<points.length;k++){
    		if(this.absMax<points[k].getCurAbsMax())
    			this.absMax = points[k].getCurAbsMax();
    	}
    	/*end计算整个聚类最大绝对上界*/
    	
    	/*start初始化对象里面的ShiMap*/
    	for(int g=0;g<points.length;g++){
    		points[g].initShiMap(this.absMax+this.absMax*0.3);//absMax参数设置。。。absMax+10?
    	}
    	/*end初始化对象里面的ShiMap*/
        	...
        	if(i!=i1&&i!=i2&&i!=i3&&i!=i4){
        		...
        		for(int j=0;j<clusters.length;j++){
        			...
        			if(shi==null){
        				...
        				if(ptc.getShi()<shi || (ptc.getShi()==shi && ptc.getB()>=valueB)){
        					if(ptc.getShi()==shi)
        						System.out.print("shi==shi============");
        					curClu = j;
        					shi = ptc.getShi();        			
        				}
        			}
    	...
    	for(int i=0;i<points.length;i++){
			...
			//	ptc.setShi();
				...
				if(shi==null){
					shi = ptc.getShi();
					valueB = ptc.getB();
				}else if(shi>ptc.getShi() || (ptc.getShi()==shi && ptc.getB()>=valueB)){
					if(ptc.getShi()==shi)
						System.out.print("shi==shi============");
					shi = ptc.getShi();
					curC = j;
...
//    }
    ...
    public double getF(double[][] N){
		...
		double F = (N[0][0]*f1+N[1][0]*f2+N[2][0]*f3+N[3][0]*f4+N[4][0]*f5+N[5][0]*f6)/214.0;
		return F;
    }
}